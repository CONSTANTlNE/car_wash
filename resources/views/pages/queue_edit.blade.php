    ...
    <section class="pb-6">
        ...
        <div class="rounded-2xl border border-[var(--color-border-light)] dark:border-[var(--color-border-dark)]
            ...
            <form class="space-y-6" action="{{ route('queue_update', $queue) }}" method="POST"
                ...
                @if ($errors->any())
                    <div
                        class="rounded-xl bg-rose-50 dark:bg-rose-900/20 border border-rose-200 dark:border-rose-800 px-4 py-3">
                ...
                <div class="flex justify-center gap-4">
                    <div>
                        <label
                            class="block text-xs font-semibold uppercase tracking-wider text-[var(--color-muted-light)] dark:text-[var(--color-muted-dark)] mb-1.5">რეცხცის
                            ტიპი</label>
                        <select name="wash_type" required class="{{ $selectClass }}">
                            <option value="">არჩევა</option>
                            @foreach ($wash_types as $washtype)
                                <option
                                    value="{{ $washtype->id }}" @selected(old('wash_type', $queue->wash_type_id) == $washtype->id)>
                                    {{ $washtype->wash_type }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <label
                            class="block text-xs font-semibold uppercase tracking-wider text-[var(--color-muted-light)] dark:text-[var(--color-muted-dark)] mb-1.5">მანქანის
                            ტიპი</label>
                ...
                <div class="flex justify-center gap-4">
                    ...
                    <div>
                        <label
                            class="block text-xs font-semibold uppercase tracking-wider text-[var(--color-muted-light)] dark:text-[var(--color-muted-dark)] mb-1.5">მფლობელის
                            მობილური</label>
                ...
                <div class="flex justify-center gap-4">
                    <div>
                        <label
                            class="block text-xs font-semibold uppercase tracking-wider text-[var(--color-muted-light)] dark:text-[var(--color-muted-dark)] mb-1.5">ბოქსი</label>
                    ...
                    <div>
                        <label
                            class="block text-xs font-semibold uppercase tracking-wider text-[var(--color-muted-light)] dark:text-[var(--color-muted-dark)] mb-1.5">თანხა</label>
                ...
                <div class="flex justify-center gap-4">
                    <div>
                        ...
                        <select name="washer" required class="{{ $selectClass }}">
                            ...
                            @foreach ($washers as $washer)
                                <option
                                    value="{{ $washer->id }}" @selected(old('washer', $queue->user_id) == $washer->id)>
                                    {{ $washer->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        <label
                            class="block text-xs font-semibold uppercase tracking-wider text-[var(--color-muted-light)] dark:text-[var(--color-muted-dark)] mb-1.5">სტატუსი</label>
                ...
                <div>
                    <label
                        class="block text-xs font-semibold uppercase tracking-wider text-[var(--color-muted-light)] dark:text-[var(--color-muted-dark)] mb-1.5">კომენტარი</label>
                ...
                <div>
                    <div class="flex flex-col justify-center items-center">
                        ...
                        @if ($existingPhoto)
                            ...
                            <a href="{{ $existingPhoto->getUrl() }}" target="_blank" id="photo-link">
                                <img id="photo-preview" src="{{ $existingPhoto->getUrl() }}" alt="car photo"
                                     class="mb-3 rounded-xl h-36 w-52 object-cover border border-[var(--color-border-light)] dark:border-[var(--color-border-dark)] cursor-pointer hover:opacity-80 transition-opacity">
                            </a>
                        @else
                    </div>
                    <a href="#" target="_blank" id="photo-link" class="hidden">
                        <img id="photo-preview" src="" alt="preview"
                             class="mb-3 rounded-xl h-36 w-52 object-cover border border-[var(--color-border-light)] dark:border-[var(--color-border-dark)] cursor-pointer hover:opacity-80 transition-opacity">
                    </a>
                    @endif
                    <input id="car-photo-input" type="file" name="car_photo" accept="image/*" capture="environment"
                           class="hidden">
                    <div class="flex items-center gap-3">
                        ...
                        <span id="photo-name"
                              class="text-xs text-[var(--color-muted-light)] dark:text-[var(--color-muted-dark)] hidden"></span>
                    </div>
                </div>
