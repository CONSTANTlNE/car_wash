                    ...
                    <tbody class="divide-y divide-[var(--color-border-light)] dark:divide-[var(--color-border-dark)]">
                        @forelse ($queues as $queue)
                            ...
                            <tr class="hover:bg-[var(--color-brand-50)] dark:hover:bg-[var(--color-brand-900)]/10 transition-colors"
                                ...
                                <td class="px-5 py-3 font-semibold text-center">₾{{ number_format($queue->wash_price, 2) }}</td>
                                <td class="px-5 py-3 text-center">
    ...
    <div id="tab-boxes" class="hidden">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            @foreach ($boxes as $box)
                ...
                <a href="{{ $activeQueue ? route('queue_edit', $activeQueue) : route('queue_create', ['box' => $box->id]) }}" class="rounded-2xl border p-5 transition-all
                    {{ $activeQueue
                    ...
                    @if(!$activeQueue)
                        <div class="flex items-center gap-2 mt-3">
                            <svg class="w-3.5 h-3.5 shrink-0 text-emerald-500 dark:text-emerald-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/>
                            </svg>
                            ...
                            <div class="flex items-center justify-between pt-2 mt-1 border-t border-rose-200 dark:border-rose-800">
                                ...
                                <div class="flex items-center gap-1.5">
                                    ...
                                    @elseif($activeQueue->status === 'done')
                                        <form action="{{ route('queue_unmark_washed', $activeQueue) }}" method="POST"
                                              onclick="event.stopPropagation()">
                                            @csrf
                                            <button type="submit"
